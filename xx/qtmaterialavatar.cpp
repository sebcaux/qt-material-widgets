#include "xx/qtmaterialavatar.h"
#include "xx/qtmaterialavatar_p.h"
#include <QPainter>
...
void QtMaterialAvatar::setLetter(const QChar &letter)
{
    ...
    d->letter = letter;
    d->type = XXMaterial::LetterAvatar;
    update();
}
...
void QtMaterialAvatar::setIcon(const QIcon &icon)
{
    ...
    d->icon = icon;
    d->type = XXMaterial::IconAvatar;
    update();
}

XXMaterial::AvatarType QtMaterialAvatar::type() const
{
    Q_D(const QtMaterialAvatar);
...
void QtMaterialAvatar::paintEvent(QPaintEvent *event)
{
    ...
    if (XXMaterial::ImageAvatar != d->type)
    {
        QBrush brush;
        brush.setStyle(Qt::SolidPattern);
